...
import { EVENT_CONFIG, isRegistrationOpen } from '../config/eventConfig';
...
// Function to get static events data (dynamic based on config)
...
function EventsSection({ onRegisterClick }: EventsSectionProps) {
  const [events, setEvents] = useState<Event[]>(getStaticEvents());
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const loadEvents = useCallback(async () => {
    // Show static events immediately (dynamic based on config)
